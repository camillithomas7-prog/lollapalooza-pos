    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
        <template x-for="o in orders" :key="o.order_id">
            <div class="card overflow-hidden" :class="cardAgeClass(o)">
                <!-- HEADER: tavolo + tempo + progresso -->
                <div class="p-4 border-b border-white/10 dark:border-white/10" :class="o.done_count === o.total_count ? 'bg-emerald-500/10' : ''">
                    ...
                    <div class="h-1.5 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
                        <div class="h-full transition-all"
                             :class="o.done_count === o.total_count ? 'bg-emerald-500' : 'bg-orange-500'"
                ...
                <div class="divide-y divide-white/5 dark:divide-white/5">
                    <template x-for="it in o.items" :key="it.id">
                        <div class="p-3 flex items-start gap-3"
                             :class="['ready','served'].includes(it.status) ? 'opacity-50 line-through' : ''">
                            <!-- Checkbox/spunta -->
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-baseline gap-2">
                                    <span class="text-2xl font-bold text-amber-500 tabular-nums" x-text="parseFloat(it.qty).toFixed(0)+'×'"></span>
                ...
                <div class="p-3 flex items-center gap-2 bg-slate-50/50 dark:bg-white/5 border-t border-white/10">
                    ...
                    <button @click="allReady(o)"
                            x-show="o.done_count < o.total_count"
...
function kdsBoard(dest){return {
    ...
    async toggleItem(it){
        // sent → preparing → ready → (loop indietro a sent se ripreme)
        const next = it.status === 'sent' ? 'preparing'
                   : it.status === 'preparing' ? 'ready'
                   : 'sent';
        it.status = next; // ottimistico
        try {
            await fetch('/api/orders.php?action=item_status',{
    ...
    async allReady(o){
        // Marca tutti gli item di questo ordine come pronti
        try {
            await fetch('/api/orders.php?action=order_all_ready',{
                method:'POST', headers:{'Content-Type':'application/json'},
                body:JSON.stringify({order_id:o.order_id, dest:this.dest})
            });
        } catch(e) {}
        this.load();
    },
