...
layout_head(t('bar'));
?>
<div class="min-h-screen p-4" x-data="kdsBoard('bar')" x-init="startPolling(); initPrinter()">
    ...
    <div x-show="syncErrors >= 3" x-cloak class="mb-3 px-4 py-2 rounded-xl bg-amber-500/20 border border-amber-500/50 text-amber-900 dark:text-amber-200 text-sm font-bold">
                    ...
                    <div class="card p-4 border-emerald-500/30 bg-emerald-500/5">
                        <div class="flex items-start justify-between gap-3 mb-3">
                            ...
                            <div class="text-right">
                                ...
                                <button @click="reopenOrder(o.order_id)" class="mt-2 px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold">↺ <?= e(t('reopen')) ?></button>
                            </div>
                        </div>
                        <ul class="text-sm space-y-1">
                            <template x-for="it in o.items" :key="it.id">
                                <li class="flex items-baseline gap-2">
                                    ...
                                    <span class="font-bold text-violet-500 tabular-nums" x-text="parseFloat(it.qty).toFixed(0)+'×'"></span>
...
function kdsBoard(dest){return {
    ...
    async load(){
        try {
            ...
            if (newCount > this.lastItemsCount && this.lastItemsCount > 0 && navigator.vibrate) navigator.vibrate([200,100,200]);
            ...
            console.warn('[KDS bar] sync error', e);
        }
    },
    async toggleItem(it){
    ...
    async reopenOrder(orderId){
        try {
            await fetch('/api/orders.php?action=order_reopen', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({order_id: orderId, dest: this.dest})});
