...
<main class="md:ml-64 lg:ml-72 p-4 md:p-8 pb-24 md:pb-8" x-data="cashPage()" x-init="load(); setInterval(load,5000)">
    <template x-if="!session">
        <div class="card p-8 text-center max-w-md mx-auto">
            ...
            <label class="text-sm text-slate-400 block mb-2">Fondo cassa iniziale (LE)</label>
    ...
    <template x-if="session">
        <div class="space-y-4">
            <div class="grid md:grid-cols-4 gap-3">
                <div class="card p-4"><div class="text-xs text-slate-400">Apertura</div><div class="font-bold text-lg" x-text="'LE '+parseFloat(session.open_amount||0).toFixed(2)"></div></div>
                <div class="card p-4"><div class="text-xs text-slate-400">Contanti</div><div class="font-bold text-lg text-emerald-400" x-text="'LE '+parseFloat(session.by_method?.cash||0).toFixed(2)"></div></div>
                <div class="card p-4"><div class="text-xs text-slate-400">Carte/POS</div><div class="font-bold text-lg text-sky-400" x-text="'LE '+(parseFloat(session.by_method?.card||0)+parseFloat(session.by_method?.pos||0)).toFixed(2)"></div></div>
                <div class="card p-4"><div class="text-xs text-slate-400">Atteso in cassa</div><div class="font-bold text-lg text-amber-400" x-text="'LE '+parseFloat(session.expected||0).toFixed(2)"></div></div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div class="card p-5">
                    ...
                    <input type="number" x-model.number="movAmt" step="0.01" placeholder="Importo LE" class="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 mb-2">
                ...
                <div class="card p-5">
                    ...
                    <input type="number" x-model.number="closeAmt" step="0.01" placeholder="LE contanti reali" class="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 mb-2 text-xl text-center">
                ...
                <tbody>
                    <template x-for="s in history" :key="s.id">
                        <tr class="border-t border-white/5">
                            ...
                            <td class="p-2 text-right whitespace-nowrap tabular-nums" x-text="'LE '+parseFloat(s.open_amount||0).toFixed(2)"></td>
                            <td class="p-2 text-right whitespace-nowrap tabular-nums" x-text="'LE '+parseFloat(s.expected||0).toFixed(2)"></td>
                            <td class="p-2 text-right whitespace-nowrap tabular-nums" x-text="s.close_amount?'LE '+parseFloat(s.close_amount).toFixed(2):'—'"></td>
                            <td class="p-2 text-right whitespace-nowrap tabular-nums font-semibold" :class="parseFloat(s.diff||0)>=0?'text-emerald-400':'text-rose-400'" x-text="s.diff!=null?'LE '+parseFloat(s.diff).toFixed(2):'—'"></td>
                        </tr>
                    </template>
                </tbody>
            </table>
