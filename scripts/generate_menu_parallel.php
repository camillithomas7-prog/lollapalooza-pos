<?php
/**
 * Versione parallela: lancia tutte le richieste OpenAI simultaneamente con curl_multi.
 */
...
echo "🚀 Lancio " . count($menu) . " richieste IN PARALLELO con curl_multi\n\n";

db()->exec("UPDATE products SET available=0 WHERE tenant_id=1");

// Filtra solo quelle che mancano o sono vuote
$pending = [];
foreach ($menu as $idx => $row) {
    $slug = trim(strtolower(preg_replace('/[^a-z0-9]+/i', '-', $row[1])), '-');
    $imgPath = "$imgDir/$slug.png";
    if (!file_exists($imgPath) || filesize($imgPath) < 1000) {
        $pending[$idx] = $row + ['slug' => $slug];
        $pending[$idx]['_slug'] = $slug;
        $pending[$idx]['_path'] = $imgPath;
    } else {
        echo "⏭  " . $row[1] . " (già presente, " . round(filesize($imgPath)/1024) . " KB)\n";
    }
}

if ($pending) {
    echo "\n🎨 Generando " . count($pending) . " immagini in parallelo...\n";
    $multi = curl_multi_init();
    $handles = [];
    foreach ($pending as $idx => $row) {
        $payload = json_encode([
            'model' => 'gpt-image-2', 'prompt' => $row[6],
    ...
    do {
        ...
        // Stream finished handles
        while ($info = curl_multi_info_read($multi)) {
            $ch = $info['handle'];
            $idx = null;
            foreach ($handles as $k => $h) if ($h['ch'] === $ch) { $idx = $k; break; }
            if ($idx === null) continue;
            $row = $handles[$idx]['row'];
            $resp = curl_multi_getcontent($ch);
            $err = curl_error($ch);
            if ($err) { echo "❌ {$row[1]}: $err\n"; }
            else {
                $j = json_decode($resp, true);
                if (isset($j['data'][0]['b64_json'])) {
                    $bin = base64_decode($j['data'][0]['b64_json']);
                    file_put_contents($row['_path'], $bin);
                    echo "✓ {$row[1]} (" . round(strlen($bin)/1024) . " KB)\n";
                } else {
                    echo "❌ {$row[1]}: " . substr($resp, 0, 200) . "\n";
                }
            }
    ...
    echo "⏱  Generazione totale: " . round(microtime(true) - $start, 1) . "s\n\n";
}

// Aggiorna DB con tutti i prodotti
echo "💾 Salvataggio DB...\n";
$ok = 0;
foreach ($menu as $row) {
    ...
    $imgUrl = "/assets/uploads/products/$slug.png";
...
echo "✅ DB aggiornato: $ok/" . count($menu) . " prodotti\n";
